...
import PracticeSettings from './settings/HospitalSettings';
...
interface ResourcesContent {
  ...
  trainingVideos: React.ReactNode;
}

interface SettingsContent {
  organizationsettings: React.ReactNode;
  hospitalsettings: React.ReactNode;
  securitysettings: React.ReactNode;
}

interface StaffManagementContent {
  allstaff: React.ReactNode;
  administrativestaff: React.ReactNode;
}
...
export default function ProfessionalDashboard() {
  ...
  const handleSidebarNavigation = useCallback(({ section, subsection, tabValue }: SidebarNavigationProps) => {
    ...
    if (section === 'Quick Access') {
      ...
      const newTabValue = `staff-${subsection.toLowerCase().replace(/\s+/g, '')}`;
      console.log("Setting staff tab value to:", newTabValue);
      setActiveTab(newTabValue);
    } else {
      const newTabValue = section === 'Resources' 
        ? `resource-${subsection.toLowerCase().replace(/\s+/g, '')}`
        : section === 'Settings'
        ? `setting-${subsection.toLowerCase().replace(/\s+/g, '')}`
        : section.toLowerCase();
      console.log("Setting tab value to:", newTabValue);
      setActiveTab(newTabValue);
    }
              ...
              <DropdownMenuContent
                ...
                >
                  <DropdownMenuLabel className="text-xs text-muted-foreground px-2 py-1.5">
                    Switch View
                  <DropdownMenuSeparator />
                  <p className="text-5l font-light tracking-tight">Welcome <span className="text-blue-500"> Golden</span>. Explore all analytic tools necessary. By clicking "Next," you confirm your intention to connect your app to Tradie API. This
                  ...
                  {data.views.map((view, index) => (
                    <DropdownMenuItem
                      ...
                      onClick={() => {
                        setActiveView(view);
                        handleViewSwitch({ name: view.name, type: view.type });
                      }}
            ...
            {activeTab.startsWith('resource-') && (
              ...
              {activeTab.startsWith('setting-') && (
                <div className="p-6 space-y-6">
                  ...
                  {activeTab === 'setting-organizationsettings' && <ProfileSettings />}
                  {activeTab === 'setting-hospitalsettings' && <PracticeSettings />}
                  {activeTab === 'setting-securitysettings' && <SecuritySettings />}
                </div>
              )}
               ...
               {/* Quick Access Projects */}
                ...
                {activeTab.startsWith('staff-') && (
                  <div className="p-6 space-y-6">
                    ...
                    {activeTab === 'staff-allstaff' && <AllStaff />}
                    {activeTab === 'staff-administrativestaff' && <AdministrativeStaff />}
                  </div>
                )}
